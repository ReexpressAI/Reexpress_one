...
struct PredictedClassSelection: View {
                ...
                VStack {
                    ...
                    VStack {
                        VStack {
                            HStack {
                                Button {
                                    documentSelectionState.resetPredictedClasses()
                                } label: {
                                    HStack(alignment: .lastTextBaseline) {
                                        Image(systemName: documentSelectionState.predictedClassesAllSelected() ? "square.inset.filled" : "square")
                        ...
                        List(selection: $documentSelectionState.predictedClasses) {
                            ForEach(0..<dataController.numberOfClasses, id:\.self) { label in
                                HStack(alignment: .firstTextBaseline) {
                                    Label("", systemImage: documentSelectionState.predictedClasses.contains(label) ? "checkmark.square.fill" : "square")
                                        .font(.title2)
                                        .foregroundStyle(.blue.gradient)
                                        .labelStyle(.iconOnly)
                                ...
                                .listRowBackground(
                                    Rectangle()
                                        ...
                                        .opacity(documentSelectionState.predictedClasses.contains(label) ? 1.0 : 0.0)
                    ...
                    .padding([.leading, .trailing])
                    .onChange(of: documentSelectionState.predictedClasses) {
                        if documentSelectionState.predictedClasses.count == 0 {
                            documentSelectionState.resetPredictedClasses()
                            withAnimation {
