//
//  TrainingProcessCacheHiddenStatesView+MFastestM1v1D.swift
//  Alpha1
//
...
extension TrainingProcessCacheHiddenStatesView {
    func cacheHiddenStates_MFastestM1v1D(batchSize: Int, datasetId: Int) async throws {
        ...
        let model = try await MFastestM1v1D.load(configuration: config)
        let reduceConfig = MLModelConfiguration()
        reduceConfig.computeUnits = .cpuAndGPU
        let reduceModel = try await MFastestM1v1DReduce.load(configuration: reduceConfig)
...
/*
 3414+1583
 Duration of Cache: 133.82117402553558
 Proccessing chunkIndex: 0
 Proccessing chunkIndex: 1000
 Duration of Cache: 64.35059297084808
 */
