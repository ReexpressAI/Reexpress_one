//
//  DataController+ML+EncDec+Interp+MFastM1v1D.swift
//  Alpha1
//
...
extension DataController {
    ...
    func forwardInterpPassEncDecBatchDual_MFastM1v1D(model: MFastM1v1D, reduceModel: MFastM1v1DReduce, keyModel: KeyModel, indexModel: KeyModel, batchSize: Int, tokenizedInputDic: [Int: SentencepieceTokenizer.DocumentTokenizationResult], multiTokenizedInputDic: [Int: SentencepieceTokenizer.DocumentTokenizationResult], padId: TokenIdType, multiPadId: TokenIdType, batchAttributes: [[Float32]]) async throws -> [Int: OutputFullAnalysisWithFeaturesPredictionType] {
        ...
        var batchedInputs: [MFastM1v1DInput] = []
        var sentencesInGroup: [Int] = []
        var batchedGroupIndex = 0
        
        var batchGroupToModelOutputArray: [[MFastM1v1DOutput]] = []
        var batchGroupToInputStructuresInGroupArray: [[MFastM1v1DInput]] = []  // Need access to array masks for reduction
        ...
        for (sentenceIndex, documentTokenizationResult) in tokenizedInputDic {
            ...
            let inputStructure = MFastM1v1DInput(input0: paddedInput.inputShapedArray, input0a: paddedInput.attentionMaskArray, input0out: decoderInput, input1: multiPaddedInput.inputShapedArray, input1a: multiPaddedInput.attentionMaskArray, input1out: decoderInput)
    ...
    func forwardInterpDualReduceEncoderHiddenStates_MFastM1v1D(modelOutput: [MFastM1v1DOutput], batchedInputs: [MFastM1v1DInput], sentencesInGroup: [Int], reduceModel: MFastM1v1DReduce, keyModel: KeyModel, indexModel: KeyModel, tokenizedInputDic: [Int: SentencepieceTokenizer.DocumentTokenizationResult], multiTokenizedInputDic: [Int: SentencepieceTokenizer.DocumentTokenizationResult], batchAttributes: [[Float32]], sentenceIndexToOutputPredictionStructure: inout [Int: OutputFullAnalysisWithFeaturesPredictionType]) async throws {
        ...
        for instanceIndex in 0..<modelOutput.count {
            var batchedReduceInputs: [MFastM1v1DReduceInput] = []
            ...
            batchedReduceInputs.append(MFastM1v1DReduceInput(output0: monoLastHiddenStateForAllTokens, output0a: attentionMask, output1: multiLastHiddenStateForAllTokens, output1a: multiAttentionMask))
            ...
            if let sentenceIndexesInDictionary = tokenizedInputDic[originalSentenceIndex]?.documentSentencesToTokenArrayIndexes.keys {
                for sentenceIndex in sentenceIndexesInDictionary {  // we expect both dictionaries to have the same keys
                    if let sentenceRangeTuple = tokenizedInputDic[originalSentenceIndex]?.documentSentencesToTokenArrayIndexes[sentenceIndex], let multiSentenceRangeTuple = multiTokenizedInputDic[originalSentenceIndex]?.documentSentencesToTokenArrayIndexes[sentenceIndex] {
                        ...
                        batchedReduceInputs.append(MFastM1v1DReduceInput(output0: monoLastHiddenStateForAllTokens, output0a: interpretabilityMaskArray, output1: multiLastHiddenStateForAllTokens, output1a: multiInterpretabilityMaskArray))
                    }
                }
            }
