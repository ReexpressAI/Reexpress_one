//
//  DataController+ML+EncDec+Interp+MFastestestM1v1D.swift
//  Alpha1
//
...
extension DataController {
    ...
    func forwardInterpPassEncDecBatchDual_MFastestM1v1D(model: MFastestM1v1D, reduceModel: MFastestM1v1DReduce, keyModel: KeyModel, indexModel: KeyModel, batchSize: Int, tokenizedInputDic: [Int: SentencepieceTokenizer.DocumentTokenizationResult], multiTokenizedInputDic: [Int: SentencepieceTokenizer.DocumentTokenizationResult], padId: TokenIdType, multiPadId: TokenIdType, batchAttributes: [[Float32]]) async throws -> [Int: OutputFullAnalysisWithFeaturesPredictionType] {
        ...
        var batchedInputs: [MFastestM1v1DInput] = []
        var sentencesInGroup: [Int] = []
        var batchedGroupIndex = 0
        
        var batchGroupToModelOutputArray: [[MFastestM1v1DOutput]] = []
        var batchGroupToInputStructuresInGroupArray: [[MFastestM1v1DInput]] = []  // Need access to array masks for reduction
        ...
        for (sentenceIndex, documentTokenizationResult) in tokenizedInputDic {
            ...
            let inputStructure = MFastestM1v1DInput(input0: paddedInput.inputShapedArray, input0a: paddedInput.attentionMaskArray, input0out: decoderInput, input1: multiPaddedInput.inputShapedArray, input1a: multiPaddedInput.attentionMaskArray, input1out: decoderInput)
    ...
    func forwardInterpDualReduceEncoderHiddenStates_MFastestM1v1D(modelOutput: [MFastestM1v1DOutput], batchedInputs: [MFastestM1v1DInput], sentencesInGroup: [Int], reduceModel: MFastestM1v1DReduce, keyModel: KeyModel, indexModel: KeyModel, tokenizedInputDic: [Int: SentencepieceTokenizer.DocumentTokenizationResult], multiTokenizedInputDic: [Int: SentencepieceTokenizer.DocumentTokenizationResult], batchAttributes: [[Float32]], sentenceIndexToOutputPredictionStructure: inout [Int: OutputFullAnalysisWithFeaturesPredictionType]) async throws {
        ...
        for instanceIndex in 0..<modelOutput.count {
            var batchedReduceInputs: [MFastestM1v1DReduceInput] = []
            ...
            batchedReduceInputs.append(MFastestM1v1DReduceInput(output0: monoLastHiddenStateForAllTokens, output0a: attentionMask, output1: multiLastHiddenStateForAllTokens, output1a: multiAttentionMask))
            ...
            if let sentenceIndexesInDictionary = tokenizedInputDic[originalSentenceIndex]?.documentSentencesToTokenArrayIndexes.keys {
                for sentenceIndex in sentenceIndexesInDictionary {  // we expect both dictionaries to have the same keys
                    if let sentenceRangeTuple = tokenizedInputDic[originalSentenceIndex]?.documentSentencesToTokenArrayIndexes[sentenceIndex], let multiSentenceRangeTuple = multiTokenizedInputDic[originalSentenceIndex]?.documentSentencesToTokenArrayIndexes[sentenceIndex] {
                        ...
                        batchedReduceInputs.append(MFastestM1v1DReduceInput(output0: monoLastHiddenStateForAllTokens, output0a: interpretabilityMaskArray, output1: multiLastHiddenStateForAllTokens, output1a: multiInterpretabilityMaskArray))
                    }
                }
            }
